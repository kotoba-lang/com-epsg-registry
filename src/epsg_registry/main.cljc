(ns epsg_registry.main
  "Kotodama WASM entrypoint for the Epsg_registry clean-room actor (L4) — Clojure port.
  ...
  facts (`epsg_registry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "epsg_registry")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"epsg_registry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epsg_registry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epsg_registry-compat/ready)
